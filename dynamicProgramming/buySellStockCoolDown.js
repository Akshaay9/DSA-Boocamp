...
const recursion = (prices, currIDX, isPossibleBuy, map) => {
  ...
  if (isPossibleBuy === 1) {
    const buy =
      -prices[currIDX] + recursion(prices, currIDX + 1, isPossibleBuy - 1, map);
    const ans = Math.max(idle, buy);
    map[currKey] = ans;
    return ans;
  } else if (isPossibleBuy === 0) {
    const sell =
      +prices[currIDX] + recursion(prices, currIDX + 2, isPossibleBuy + 1, map);
    const ans = Math.max(idle, sell);
    map[currKey] = ans;
    return ans;
  }
};
console.log(recursion(prices, 0, 1, {}));
